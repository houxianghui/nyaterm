  ...
  const renderTabItem = (tab: Tab, index: number) => {
    ...
    const tabButton = (
      <div
        ...
        className={`group relative flex h-7 shrink-0 items-center gap-1.5 rounded-full px-3 text-xs font-medium transition-[color,background-color,box-shadow,opacity] duration-200 ${
          isActive ? "" : "df-hover"
        } ${draggedTabId === tab.id ? "opacity-60" : ""}`}
        style={{
          backgroundColor: isActive ? "var(--df-bg-panel)" : undefined,
          boxShadow: isActive
            ? "0 1px 3px rgb(0 0 0 / 0.12), 0 0 0 1px var(--df-border)"
            : undefined,
